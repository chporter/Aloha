...
C=======================================================================
     ...
     &           DSSATP,FILEWT,RSEED1)
      ...
      INTEGER      MENU,NLOOP,PATHL,YEAR,RSEED1
      LOGICAL      FEXIST
      REAL         WTHADJ(2,8)
...
C
      ...
      ELSE IF (MENU .EQ. 1) THEN
          ...
          IF (MEWTH .EQ. 'M') THEN
             IF (FILEW(11:11) .EQ. BLANK) THEN
                ...
                WRITE (WTHTEM(5:12),170) YEAR,'01.WTH'
              ELSE
           ...
           ELSE IF (MEWTH .EQ. 'G') THEN
             IF (FILEW(11:11) .EQ. BLANK) THEN
                ...
                WRITE (WTHTEM(5:12),170) YEAR,'01.WTG'
              ELSE
      ...
      ELSE IF (MENU .EQ. 3) THEN
          ...
          PATHL  = INDEX (PATHWT,BLANK)
          IF (PATHL .GT. 1) THEN
             IF (PATHWT((PATHL-1):(PATHL-1)) .NE. '\') THEN
                WRITE (PATHWT(PATHL:PATHL),'(A1)') '\'
C-SUN        IF (PATHWT((PATHL-1):(PATHL-1)) .NE. '/') THEN
C-SUN           WRITE (PATHWT(PATHL:PATHL),'(A1)') '/'
                PATHL = PATHL+1
             ENDIF
          ENDIF
      ELSE IF (MENU .EQ. 4) THEN
          IF (RNMODE .EQ. 'I') WRITE (*,600) FILEW
          READ (5,'(A12)') WTHTEM
          IF (PATHL .LE. 1) THEN
             FILEXX = WTHTEM
           ELSE
...
C=======================================================================
      ...
      CHARACTER*1  ANS,RNMODE,LINE(80),BLANK
      CHARACTER*4  YEARW
      CHARACTER*6  ERRKEY
      CHARACTER*7  FILEWT
      ...
      INTEGER      I,NWTH,ERRNUM,NLOOP,PATHL,LUNLST,ISECT,LINF
      LOGICAL*4    FEXIST
...
C-----------------------------------------------------------------------
  ...
  400 FORMAT (1X,I3,')',1X,A25,2(1X,F8.3),1X,F6.0,2X,A4,3X,A12)
  500 FORMAT (/,6X,'More.... PRESS ENTER KEY')
  900 FORMAT (/,6X,'WEATHER DATA SELECTED ===>',1X,I3,
     &        /,6X,'ALTERNATE SELECTION ? --->',2X,' ',$)
  950 FORMAT (/,6X,
